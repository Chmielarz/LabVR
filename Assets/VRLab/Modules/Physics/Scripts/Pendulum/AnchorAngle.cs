...
namespace PhysicsLab
{
    ...
    {
        private float angle = 0.0f;        // @TODO the player should be able to set the angle with his hands
        ...
        {
            float b = Math.Abs(transform.position.y - cube.position.y);
            float a = Math.Abs(transform.position.x - cube.position.x);
